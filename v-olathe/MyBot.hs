...
targetsForSource
  :: Planet
     ...
     -> [(ShipCount, (Planet, Planet, (Double, ShipCount)))]
targetsForSource src candidates = 
  let
    ...
    sc2_candidates' = sortBy rank 
                      $ filter (\(_,_,(_,shipsDst)) -> shipsDst < srcAvail)  
                      $ fp_candidates
    ...
    debugStr = "targetsForSource:candidates=" ++ show(candidates)
    --debugStr = "fp_candidates="++show(fp_candidates)
    --debugStr = "sc2_candidates'="++show(sc2_candidates')
...
score2 src dst owner cnt
  ...
  where
    ...
    cntFuture = cnt + ((2*(dist * (production dst))) `div` 3)
